...
const ForgotPassword = () => {
    ...
    const onSubmit = (data) => {
        const endpoint = data.role === 'boss' 
            ? '/boss/password/recover' 
            : '/employees/password/recover';
        const url = `${import.meta.env.VITE_API_URL}${endpoint}`;
        fetchDataBackend(url, { email: data.email }, 'POST');
    };

    return (
        <div className="flex flex-col sm:flex-row h-screen bg-[var(--color-white)]">
            <ToastContainer />

            {/* Left Panel - Form */}
            <div className="w-full sm:w-1/2 h-screen flex justify-center items-center p-4">
                <div className="w-full max-w-md">
                    <h1 className="text-3xl font-semibold mb-2 text-[var(--color-obsidian)]">
                        Recuperar contraseña
                    </h1>
                    <p className="text-[var(--color-obsidian)] opacity-80 block my-4 text-center">
                        Ingresa tu correo electrónico y selecciona tu rol para recibir instrucciones
                    </p>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        <button
                            type="submit"
                            className="w-full bg-blue-velvet text-white  py-2 px-4 rounded-md hover:bg-blue-700 hover:outline-0 transition duration-300 mt-6"
                        >
                            Recuperar contraseña
                        </button>
                    </form>
                    ...
                    <div className="mt-6 ">
                        <Link to="/login" className="flex items-center text-blue-velvet/80 mb-6 hover:text-blue-800 hover:scale-105 transition-transform duration-300">
                        <svg
            ...
            {/* Right Panel - Image */}
            <div className="hidden relative sm:block w-1/2 h-screen bg-[var(--color-obsidian)]">
            ...
            <div 
                ...
                <div className="h-full flex items-center justify-center p-12">
                    <div className="text-white text-center">
                        <h2 className="text-4xl font-bold mb-4">¿Problemas para ingresar?</h2>
                        <p className="text-xl opacity-90">
                            Sigue los pasos y recupera el acceso a tu cuenta en minutos
                        </p>
                    </div>
                </div>
            </div>
